...
- (void)viewDidLoad
{
    ...
    [self setViewControllers:@[startViewController] direction:UIPageViewControllerNavigationDirectionForward
                    animated:NO
                  completion:nil];
...
- (void)createContentDataArray {
    ...
    for (int i = 0; i < 4; ++i) {
        contentData[i].title = ((NSString *)titles[i]).localize;
        contentData[i].contentText = ((NSString *)contents[i]).localize;
